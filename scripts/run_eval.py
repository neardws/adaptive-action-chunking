...
def obs_to_batch(obs, task_lang: str, device: str):
    ...
    state_keys = [k for k in obs if "joint" in k or "gripper" in k or "eef" in k]
    state_parts = [obs[k] if isinstance(obs[k], np.ndarray) else np.array([obs[k]])
                   for k in sorted(state_keys)]
    if state_parts:
        state = np.concatenate(state_parts)
    else:
        state = np.zeros(32)
    state = state[:32]  # truncate/pad to 32
    if len(state) < 32:
